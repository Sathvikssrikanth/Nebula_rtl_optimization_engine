/*  SPI :
    It provides register-based control for SPI configuration, data transmission, and status monitoring in both master and slave modes.
    It supports configurable clock generation using baud rate generator (BAUD_GEN) and handles clock phase and polarity (CPHA, CPOL) via PHASE_POLARITY module.
    On receiving data in the Rx buffer, it generates an interrupt which goes to the RISC-V module via APB
*/
...
module spi (
    ...
    always@(*)begin
        pready = 1'b0;  // default - the (read_write && !penable) case previously
                         // left pready unassigned, inferring a latch
        if(read_write)begin
            if(penable)begin
                case(address[4:0])
